...
for i, (m, n) in zip((3,2,1,0), zip((prehex75, hex75, prehex92, 56     rowmax = pd.DataFrame([np.max(f, axis=0) for f in m])
    # convert frames to seconds
    rowmax.index = [int(i) for i in rowmax.index*20/60]
    ax = axes.flat[i]
    sns.heatmap(rowmax, ax=ax, cmap='viridis', xticklabels=0,
        yticklabels=6, vmin=100, vmax=1500 if '75' in n else 75 62     ax.set_title(n)
    ax.set_yticklabels(ax.get_yticklabels(), rotation=30, fonts 64 axes[0,0].axhline(5, ls='--', alpha=0.4, c='w')
