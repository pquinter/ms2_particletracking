...
peaks.to_csv('../output/111617_TL47PP7smFish.csv', index=False)
...
with pm.Model() as model_TS:
    # Priors
    # estimate of mu_t
    #mu_t, beta = df_mcmc_mrna[['mu_t','beta']].apply(np.median)
    #mu_obs = pm.Cauchy('mu_t', alpha=mu_t, beta=beta)
    ...
    # Likelihood
    a_obs = pm.Normal('a_obs', mu=alpha*mu_obs, sd=sigma_a, observed=TSint, shape=len(TSint))
    trace = pm.sample(draws=2000, tune=2000, init='advi+adapt_diag', njobs=4)
...
def plot_ecdf(arr):
    plt.figure()
    plt.scatter(*ecdf(arr), s=15, alpha=0.3)
